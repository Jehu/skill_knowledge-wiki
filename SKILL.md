...
| Substack Note `--use-browser` verliert Comments/Reactions | `fetch_render.js` selektiert nur `feedPermalinkUnit` → Note-Text ohne Comments. Substack liefert Comments aber im SSR-Initial-HTML. **Ohne** `--use-browser` bekommt `fetch_url()` die komplette Seite inklusive Comments. Zudem setzt `_extract_content()` trafilatura mit `include_tables=False` → Tabellen fliegen raus. Siehe Trigger-Tabelle oben für Trade-off. |
| trafilatura `include_tables=False` killt Struktur-Daten | `_extract_content()` in `ingest_source.py` ruft trafilatura mit `include_tables=False` auf. Für Substack Notes mit Kommentar-Tabellen oder strukturierten Daten die Ausgabe reduzieren. Fix: `--use-browser` weglassen für Notes mit Comments. |
